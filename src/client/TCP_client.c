...
#include "../../inc/client/TCP_client.h"
...
int InitialisationAvecService(char *machine, char *service) {
	...
	if ( (n = getaddrinfo(machine, service, &hints, &res)) != 0)  {
     		fprintf(stderr,RED ">>> Initialisation, erreur de getaddrinfo : %s\n" RESET, gai_strerror(n));
     		return 0;
	}
	ressave = res;
	...
	if (res == NULL) {
			freeaddrinfo(ressave);
     		printf(RED ">>> Initialisation, erreur de connection\n" RESET);
     		return 0;
	}

	freeaddrinfo(ressave);
	printf(GRN ">>> Connexion avec le serveur reussie.\n" RESET);
...
char message[LONGUEUR_TAMPON];
	...
	while(!fini) {
		...
		if ((index > 0) && (tamponClient[debutTampon]=='\n')) {
			...
			if (retour < 0) {
				perror(RED ">>> Reception, erreur de recv\n" RESET);
				return NULL;
			} else if(retour == 0) {
				fprintf(stderr,MAG ">>> Reception, le serveur a ferme la connexion.\n" RESET);
				return NULL;
			} else {
				/*
...
int ReceptionBinaire(char *donnees, size_t tailleMax) {
	...
	if(dejaRecu < tailleMax) {
		...
		if(retour < 0) {
			...
			fprintf(stderr,MAG ">>> ReceptionBinaire, le serveur a ferme la connexion.\n" RESET);
			return 0;
		} else {
			/*
...
int EmissionBinaire(char *donnees, size_t taille) {
	...
	if(retour == -1) {
		perror(RED ">>> Emission, probleme lors du send\n" RESET);
		return -1;
	} else {
		return retour;
	}
